...
func prepareAvatar(reader io.Reader, size int64, contentType string) (preparedAvatar, error) {
	if size <= 0 {
		return preparedAvatar{}, fmt.Errorf("avatar is empty")
	}
	data, err := io.ReadAll(io.LimitReader(reader, size+1))
	if err != nil {
		return preparedAvatar{}, fmt.Errorf("read avatar: %w", err)
	}
	if int64(len(data)) != size {
		return preparedAvatar{}, fmt.Errorf("avatar size mismatch")
	}

	cfg, _, err := image.DecodeConfig(bytes.NewReader(data))
	if err != nil {
		return preparedAvatar{}, fmt.Errorf("decode avatar config: %w", err)
	}
	if cfg.Width <= 0 || cfg.Height <= 0 || int64(cfg.Width)*int64(cfg.Height) > AvatarMaxPixels {
		return preparedAvatar{}, fmt.Errorf("avatar dimensions are invalid or too large")
	}

	img, _, err := image.Decode(bytes.NewReader(data))
	if err != nil {
		return preparedAvatar{}, fmt.Errorf("decode avatar: %w", err)
	}
	...
	case "image/jpeg":
		if err := jpeg.Encode(&out, dst, &jpeg.Options{Quality: avatarJPEGQuality}); err != nil {
			return preparedAvatar{}, fmt.Errorf("encode resized jpeg avatar: %w", err)
		}
	case "image/png":
		if err := png.Encode(&out, dst); err != nil {
			return preparedAvatar{}, fmt.Errorf("encode resized png avatar: %w", err)
		}
	case "image/webp":
		outType = "image/png"
		if err := png.Encode(&out, dst); err != nil {
			return preparedAvatar{}, fmt.Errorf("encode resized webp avatar as png: %w", err)
		}
	default:
		return preparedAvatar{}, fmt.Errorf("unsupported avatar content type %q", contentType)
	}
