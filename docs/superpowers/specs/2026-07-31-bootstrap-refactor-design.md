...
- Дублирование `/health` и `/livez` — нужно сперва выяснить, что читают k8s-пробы и Prometheus.
...
- `/health` (main.go:235) и `/livez` (main.go:415) возвращают разный JSON при одинаковом смысле.
  Кто именно их читает — k8s-проба, Prometheus, что-то ещё? До ответа оба остаются как есть.
- Нужен ли `cmd/migrate` в `Dockerfile.server` (или отдельным образом) для прод-миграций, или
  это осознанно ручная операция.
