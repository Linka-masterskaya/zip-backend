...
- Числовой префикс и имя файла миграции неизменяемы после мержа. Смена timestamp создаёт для goose новую версию и приводит к повторной накатке.
- Порядок разбора текущего затора: **#89 → #88 → #92**. #89 задаёт схему, #88 трогает `pkg/linka` и одну строку в `service.go`, #92 переписывает `internal/pack` целиком.
...
**Права:** публиковать может `defectologist` и `admin`. `viewer` и `parent` — нет.
